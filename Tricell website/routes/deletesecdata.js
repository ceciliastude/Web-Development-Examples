...
router.get('/:id', function(request, response)
{
    ...
    async function sqlQuery()
    {
        ...
        if(request.session.loggedin)
        {
            ...
            const result = await connection.execute("DELETE FROM researchObjects WHERE id="+id+"");

            // Ge respons till användaren
            response.write("Virus information deleted<br/><p /><a href=\"http://localhost:3000/api/researchdata\" style=\"color:#336699;text-decoration:none;\">Delete more virus information</a>");
        }
        else
        {
